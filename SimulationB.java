...
public class Simulation {
  ...
  public static Queue<ClientCalc> queue1 = new ArrayDeque<ClientCalc>();
 ...
 static void init(String[] args) {
    if (args.length<4) {
      System.out.println("Usage: java Simulation Multiple Queue Multiple Servers ");
    ...
    randomArrival = new ExponentialRandom(meanInterarrivalTime);
      ...
      for (int t=0, clientId=0; clientId < numClients; t++) {
        if (t == nextArrivalTime) {
          ...
          System.out.println("\tClient queue: " + queue1);
        ...
        for (ServerCalc server : servers) {
          ...
          if (server.isIdle() && !queue1.isEmpty()) {
            ClientCalc client = (ClientCalc)queue1.remove();
